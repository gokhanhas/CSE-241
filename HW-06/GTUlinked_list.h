...
#ifndef _GTU_LINKED_LIST
#define _GTU_LINKED_LIST
...
namespace MY_STL{
	...
	class GTUlinked_list : public GTUqueue<E,container_type> , public GTUlist<E,container_type>{

		public:
			
			GTUlinked_list() noexcept { /* Body Initialize Empty */}
			
			GTUlinked_list(std::list<E> newContainer) noexcept {
			...
			void add(E e) noexcept { 
				...
				// Note that queue is not sort function
			}
			...
			E poll() throw() { 
				...
				try{
					if(this->size() == 0)
						...
						E temp = *(listContainer.begin());
						listContainer.erase(listContainer.begin());
						return temp;
					}
				} catch(int){
